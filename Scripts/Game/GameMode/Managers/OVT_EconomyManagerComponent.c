...
//	}
	...
	// --- SPARK Modified: Changed to sum money from all wallets in player's inventory ---
	int GetPlayerMoney(string playerId)
	{
		int playerIdInt = OVT_Global.GetPlayers().GetPlayerIDFromPersistentID(playerId);
		if (playerIdInt == -1) return 0;
		
		IEntity playerEntity = GetGame().GetPlayerManager().GetPlayerControlledEntity(playerIdInt);
		if (!playerEntity) return 0;

		InventoryStorageManagerComponent inventory = InventoryStorageManagerComponent.Cast(playerEntity.FindComponent(InventoryStorageManagerComponent));
		if (!inventory) return 0;

		array<IEntity> wallets = {};
		inventory.GetItems(wallets);
		
		int totalMoney = 0;
		foreach (IEntity wallet : wallets)
		{
			WalletComponent walletComp = WalletComponent.Cast(wallet.FindComponent(WalletComponent));
			if (walletComp)
			{
				totalMoney += walletComp.GetBalance();
			}
		}
		return totalMoney;
	}
...
//	}
	...
	// --- SPARK EDIT Start Change: Updated PlayerHasMoney to use wallet system instead of player.money ---
	// Previously checked player.money directly, now calculates total money from wallets
	bool PlayerHasMoney(string playerId, int amount)
	{
...
//	}
	...
	// --- Modified: Now adds money to a wallet, spawning one if necessary ---
	void AddPlayerMoney(int playerId, int amount, bool doEvent=false)
	{
		if (Replication.IsServer())
		{
	...
	// SPARK Modified: --- Helper function to get or create a wallet ---
	// Function to get or create a wallet for the player
	protected WalletComponent GetOrCreateWallet(IEntity playerEntity)
	{
	    // Check if the player has an inventory component
	    InventoryStorageManagerComponent inventory = InventoryStorageManagerComponent.Cast(playerEntity.FindComponent(InventoryStorageManagerComponent));
	    if (!inventory)
	    {
	        Print("Error: Player has no InventoryStorageManagerComponent for player: " + playerEntity.GetName());
	        return null;
	    }
	
	    // Look for an existing wallet in the player's inventory
	    array<IEntity> wallets = {};
	    inventory.GetItems(wallets);
	    foreach (IEntity wallet : wallets)
	    {
	        ...
	        if (walletComp)
	        {
	            Print("Found existing wallet in player's inventory for player: " + playerEntity.GetName());
	            return walletComp;
	        }
	    }
	
	    // No wallet found, create a new one
	    ResourceName walletPrefab = "{5B6E140207B292F2}Prefabs/Custom/StorageUnit/Crypto_Wallet.et";
	    Resource walletResource = Resource.Load(walletPrefab);
	    if (!walletResource.IsValid())
	    {
	        Print("Failed to load wallet prefab: " + walletPrefab);
	        return null;
	    }
	
	    BaseWorld world = GetGame().GetWorld();
	    EntitySpawnParams spawnParams = new EntitySpawnParams();
	    spawnParams.Transform[3] = playerEntity.GetOrigin();
	
	    IEntity newWallet = GetGame().SpawnEntityPrefab(walletResource, world, spawnParams);
	    if (!newWallet)
	    {
	        Print("Failed to spawn wallet entity for player: " + playerEntity.GetName());
	        return null;
	    }
	
	    // Get and initialize the WalletComponent
	    WalletComponent walletComp = WalletComponent.Cast(newWallet.FindComponent(WalletComponent));
	    if (!walletComp)
	    {
	        Print("WalletComponent not found on spawned wallet for player: " + playerEntity.GetName());
	        SCR_EntityHelper.DeleteEntityAndChildren(newWallet);
	        return null;
	    }
	

	
	    // Insert the wallet into the player's inventory
	    if (!inventory.TryInsertItem(newWallet))
	    {
	        Print("Failed to insert wallet into inventory for player: " + playerEntity.GetName());
	        SCR_EntityHelper.DeleteEntityAndChildren(newWallet);
	        return null;
	    }
	
	    Print("New wallet added to inventory for player: " + playerEntity.GetName());
	    return walletComp;
	}
		// --- End Modification ---
	...
	//	}
		...
		// SPARK Modified
		// Function to add money to the player's wallet (called when selling an item)
	void DoAddPlayerMoney(int playerId, int amount)
	{
	    ...
	    if (!playerEntity)
	    {
	        Print("Error: No player entity found for ID: " + playerId);
	        return;
	    }
	
	    // Get or create the wallet
	    WalletComponent wallet = GetOrCreateWallet(playerEntity);
	    if (!wallet)
	    {
	        Print("Error: Could not get or create wallet for player ID: " + playerId);
	        return;
	    }
	
	    // Add the money to the wallet
	    wallet.AddBalance(amount);
	    Print("Success: Added " + amount + " to player ID: " + playerId + "'s wallet.");
	
	    // Notify the system of the money change
	    string persId = OVT_Global.GetPlayers().GetPersistentIDFromPlayerID(playerId);
	    m_OnPlayerMoneyChanged.Invoke(persId, wallet.GetBalance()); // Assumes GetBalance returns the updated amount
	}
	// --- End Modification ---
...
//	}
	...
	// --- Spark Modified: Now deducts money from wallets ---
	void TakePlayerMoney(int playerId, int amount)
	{
		if (Replication.IsServer())
		{
...
//	}
	...
	// --- Spark Modified
	// Updated TakePlayerMoneyPersistentId to use wallet system ---
	// Removed direct manipulation of player.money, now calls DoTakePlayerMoney
	void TakePlayerMoneyPersistentId(string persId, int amount)
	{
		int playerId = OVT_Global.GetPlayers().GetPlayerIDFromPersistentID(persId);
...
//	}
	...
	// --- Spark Modified
	// Updated AddPlayerMoneyPersistentId to use wallet system ---
	// Removed direct manipulation of player.money, now calls DoAddPlayerMoney
	void AddPlayerMoneyPersistentId(string persId, int amount)
	{
		int playerId = OVT_Global.GetPlayers().GetPlayerIDFromPersistentID(persId);
		if (playerId == -1) return;
...
//	}
	...
	void DoTakePlayerMoney(int playerId, int amount)
	{
		...
		foreach (IEntity wallet : wallets)
		{
			...
			if (walletComp)
			{
				int balance = walletComp.GetBalance();
				if (balance >= remaining)
				{
					walletComp.RemoveBalance(remaining);
					remaining = 0;
					break;
				}
				else
				{
					walletComp.SetBalance(0);
					remaining -= balance;
				}
			}
		}
		...
		string persId = OVT_Global.GetPlayers().GetPersistentIDFromPlayerID(playerId);
		m_OnPlayerMoneyChanged.Invoke(persId, GetPlayerMoney(persId));
	}
	// --- End Modification ---
...
//	}
	...
	// --- Start Change: Updated StreamPlayerMoney to remove player.money reference ---
	// Removed direct streaming of player.money, now unnecessary as wallet balances are replicated
	void StreamPlayerMoney(int playerId)
	{
		// No longer needed since wallet components handle replication
		// string persId = OVT_Global.GetPlayers().GetPersistentIDFromPlayerID(playerId);
		// OVT_PlayerData player = OVT_Global.GetPlayers().GetPlayer(persId);
		// if(!player) return;
		// Rpc(RpcDo_SetPlayerMoney, playerId, player.money);
...
//	}
	...
	// --- Start Change: Updated RpcDo_SetPlayerMoney to remove player.money manipulation ---
	// Made obsolete as wallet balances are managed and replicated by WalletComponent
	[RplRpc(RplChannel.Reliable, RplRcver.Broadcast)]
	protected void RpcDo_SetPlayerMoney(int playerId, int amount)
	{
		// Obsolete with wallet system, kept for compatibility but does nothing
		// string persId = OVT_Global.GetPlayers().GetPersistentIDFromPlayerID(playerId);
		// OVT_PlayerData player = OVT_Global.GetPlayers().GetPlayer(persId);
		// if(!player) return;
		// player.money = amount;
